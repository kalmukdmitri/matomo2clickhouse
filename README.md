...
### ВНИМАНИЕ!
- в переменной settings.tables_not_updated указаны таблицы, для которых все UPDATE заменены на INSERT, т.е. записи добавляются, а не изменяются. Это необходимо учитывать при селесах! Актуланые записи - те, у которых максимальное значение dateid

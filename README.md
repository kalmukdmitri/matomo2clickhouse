...
Нужно в файле ```settings.py``` заменить значения параметров так: ```DEBUG = True```, ```EXECUTE_CLICKHOUSE = True```, ```replication_batch_sql = 0```.
Запустить скрипт вручную. В самой последней строке, содержащей переменную ```dv_sql_for_execute_last``` будет тело "проблемного" запроса. Необходимо разобраться что с ним не так и принять меры.
